...
**Automated:** Homebrew, all packages, Volta/Node/npm globals, Rustup, Go tools, pipx, Neovim clone, git-delta config, fzf keybindings, macOS defaults, zshrc + aerospace.toml + SketchyBar + Karabiner-Elements config symlinks, SketchyBar service start (Catppuccin Mocha config included), JankyBorders launch via AeroSpace startup.
